...
object PositionService {
  def defaultPosition: Position = new Position(0, 0, N())

  def reposition(move: Move, position: Try[Position], cityMap: MapLimits, name: String): Try[Position] = move match {
  ...
  def advance(position: Position, cityMap: MapLimits): Try[Position] = position.o match {
