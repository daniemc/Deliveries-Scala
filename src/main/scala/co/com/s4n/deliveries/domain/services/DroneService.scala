...
import co.com.s4n.deliveries.domain.VO.{Move, N}
...
object DroneService {
  def defaultDrone: Drone = new Drone("", "", PositionService.defaultPosition)

  def output(name: String): String =s"out${name}.txt"

  def prepareDrone(name: String) : Try[Drone] = Try(new Drone(name, output(name), PositionService.defaultPosition))

  def move(movement: Try[Move], drone: Try[Drone], cityMap: MapLimits): Try[Drone] = movement match {
  ...
  def goToNewPosition(step: Move, drone: Try[Drone], cityMap: MapLimits) = {
